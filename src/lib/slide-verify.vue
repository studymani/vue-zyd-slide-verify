<template>
    <div class="slide-verify" id="slideVerify" onselectstart="return false;">
        <canvas v-if="showSlideOnly == false" :width="w" :height="h" ref="canvas" ></canvas>
        <div v-if="showSlideOnly == false" @click="refresh" class="slide-verify-refresh-icon"></div>
        <canvas v-if="showSlideOnly == false" :width="w" :height="h" ref="block"  class="slide-verify-block"></canvas>
        <!-- container -->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 随机生成img src
            getRandomImg() {
                return '../assets/img_code_' + this.getRandomNumberByRange(1, 20)+'.jpg';
